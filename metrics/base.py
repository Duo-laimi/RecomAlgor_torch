...
class AbstractMetric(object):
    ...
    def calculate_metric(self, data_object):
        ...
        Args:
            data_object(DataStruct): it contains all the information needed to calculate metrics.
...
class LossMetric(AbstractMetric):
    ...
    def used_info(self, data_object):
        ...
        preds = data_object.get('rec.score')
        trues = data_object.get('data.label')

        return preds.squeeze(-1).numpy(), trues.squeeze(-1).numpy()

    def output_metric(self, metric, data_object):
        preds, trues = self.used_info(data_object)
